...
#else
#include <GL/glut.h>
#endif/home/mosarvit/Documents/1333/444//.objs
...
//    int rem=field.calculateRemainingMines();
    int rem=field.mc.remainingMines;
    ...
    ////////////////////////////
    // number of remaining supermines

    glColor3f(0,0,1);

    rem=field.mc.remainingSupermines;

    if (rem>999) rem=999;

    int dy=BORDER_WIDTH+DISPLAY_BORDER_WIDTH;
    int dx=BORDER_WIDTH+58+DISPLAY_BORDER_WIDTH;

    for (int i=0;i<3;i++) {
        int digit=rem%10;
        rem/=10;
        drawDigit(digit,32-16*i+dx,dy,2);
    }


    glColor3f(0,0,0);
    drawRect(BORDER_WIDTH+58,
            BORDER_WIDTH,
            48+DISPLAY_BORDER_WIDTH,
            24+DISPLAY_BORDER_WIDTH);
    ...
    dx=originalWidth-BORDER_WIDTH-16;
    dy=BORDER_WIDTH+DISPLAY_BORDER_WIDTH;
...
//                if (!field.allSquares[x][y].isCovered) {
                if (!field.allSquares[x][y].state) {
                    float zoom=1.5*squareSize/35;



                if ((!field.allSquares[x][y].state)and(field.allSquares[x][y].surroundingMines)) {


                switch(field.allSquares[x][y].surroundingMines) {
                    ...
                    drawDigit(field.allSquares[x][y].surroundingMines,x1+.25*squareSize-1.5*zoom, y1+.5*squareSize-5.0*zoom,zoom);
                }

                if (field.allSquares[x][y].surroundingSupermines) {

                    glColor3f(0,0,1);



                        drawDigit(field.allSquares[x][y].surroundingSupermines,x1+.75*squareSize-1.5*zoom,
                ...
                /*else if (field.state[x][y]==9*/
                else if (field.allSquares[x][y].state==1
                    ...
                    glEnd();
                /*}else if (field.state2[x][y]==9*/
                }else if (field.allSquares[x][y].state==1
                         and (gameState==GAME_LOST or gameState==GAME_WON /* or true*/)
                         and field.isSupermine(x,y)
                        ) {
                    ...
                    glEnd();
                         ...
                         and field.allSquares[x][y].adjacentToSuperFlag
                        ) {
                ...
                /* else if (field.state[x][y]==10) {  // flag */
                else if (field.allSquares[x][y].state==2) {  // flag
                    ...
                    if (field.allSquares[x][y].adjacentToSuperFlag==1){
                        glColor3f(.7,.7,.95);
                        glBegin(GL_TRIANGLES);
                        glVertex2f(x1,y1);
                        glVertex2f(x2,y1);
                ...
                else if (field.allSquares[x][y].state==3) {  // superflag
    ...
    if (field.allSquares[hitMineX][hitMineY].mine==2){

//        cout << "Hello1" << endl;

        vector<Square*> hitMines = field.surroundingSquares(hitMineX, hitMineY, 1);

        for (vector<Square*>::iterator it = hitMines.begin(); it != hitMines.end(); it++){

            hitMineDisplayX1=FIELD_X+((*it)->posX+0)*squareSize;
            hitMineDisplayX1=FIELD_X+((*it)->posX+0)*squareSize;
            hitMineDisplayX2=FIELD_X+((*it)->posX+1)*squareSize;
            hitMineDisplayY1=FIELD_Y+((*it)->posY+0)*squareSize;
            hitMineDisplayY2=FIELD_Y+((*it)->posY+1)*squareSize;
...
void endGameLost() {
    ...
    newScore.mines=field.mc.remainingMines;
    newScore.supermines=field.mc.remainingSupermines;
...
int main(int argc, char** argv) {
    ...
    field.height=16;
    field.width=16;
    field.mc.setStartMines(40);
    field.mc.setStartSupermines(10);
    ...
    while ((option_char = getopt (argc, argv, "d:s:w:h:m:n:p:t3f:c:g:il:")) != -1)
        switch (option_char) {
            ...
            case 'm':
                difficulty=4;
                field.mc.setStartMines(atoi(optarg));
                field.mc.setStartSupermines(2);
                break;
            case 'w':
                difficulty=4;
        ...
        if (listScoresType!=0) { // list scores
            ...
            switch(difficulty) {
                case 1:
                    field.height=9;
                    field.width=9;
                    field.mc.setStartMines(10);
                    field.mc.setStartSupermines(5);
                    break;
                case 2:
                    field.height=16;
                    field.width=16;
